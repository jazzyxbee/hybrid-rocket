...
double hybrid_rocket_thrust(double RI, double L, double T, double MOL, double kappa, double Pc, int E, double mDot_o) {
    ...
    A_b = 2 * M_PI * RI * L + 2 * M_PI * pow(RI,2);

    G_o = (mDot_o) / (M_PI * pow((RI), 2));

    rDot = a * pow(G_o, n); // millimeters per second should be 0.56 to 0.72?

    mDot_f = Pf * A_b * rDot;

    //mDot = mDot_o + mDot_f;
    mDot = 34.41; ///ignoring the math above as I know the mass flow rate is the mass over the seconds.
    ...
    v_e = sqrt(((2 * kappa*9.81) / (kappa - 1)) * (R * T/MOL) * (1.0 - pow((Pe / Pc), ((kappa - 1) / (kappa))))); // =770.9 ms^2 validated 3.3. ISENTROPIC FLOW THROUGH NOZZLES PG 55

    Thrust = mDot * v_e;

    double Ip = v_e;  /// in vaccum g0 = 1;
